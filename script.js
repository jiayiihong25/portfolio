...
/* ==========================================================================
   Constellation Star Drift (Stage 3)
   Each star wanders independently on its own sine path. The connected
   <line> elements are redrawn from the same coordinates every frame, so a
   line can never separate from the stars it connects — unlike a CSS
   transform on the star alone, which has no way to also move the line.
   ========================================================================== */

function initConstellationDrift() {
    const svg = document.getElementById('constellation-svg-graphic');
    if (!svg) return;

    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduceMotion) return;

    const stars = [...svg.querySelectorAll('.constellation-star')].map(el => {
        const baseX = parseFloat(el.getAttribute('cx'));
        ...
        return {
            ...
            curX: baseX,
            curY: baseY,
            ampX: 2 + Math.random() * 2.5,
            ampY: 2 + Math.random() * 2.5,
            speedX: 0.35 + Math.random() * 0.5,
    ...
    const lines = [...svg.querySelectorAll('.constellation-line')].map(el => ({
        el,
    ...
    function tick(now) {
        const t = now / 1000;
        stars.forEach(s => {
            s.curX = s.baseX + Math.sin(t * s.speedX + s.phaseX) * s.ampX;
            s.curY = s.baseY + Math.cos(t * s.speedY + s.phaseY) * s.ampY;
            s.el.setAttribute('cx', s.curX);
            s.el.setAttribute('cy', s.curY);
        });
        lines.forEach(l => {
            const a = stars[l.a];
            const b = stars[l.b];
            if (!a || !b) return;
            l.el.setAttribute('x1', a.curX);
            l.el.setAttribute('y1', a.curY);
            l.el.setAttribute('x2', b.curX);
            l.el.setAttribute('y2', b.curY);
        });
...
/* ==========================================================================
   Constellation Hover/Tap Interaction System (Stage 3)
   ========================================================================== */

function initConstellationInteractions() {
    const graphicGroup = document.getElementById('constellation-graphic');
    if (!graphicGroup) return;

    // Stars: hover previews a piece, click locks it in the preview panel.
    // Only one star's image is ever shown at a time. The panel pops up right
    // beside whichever star is active, tilted (like the about-me photos and
    // the cases-page thumbnails) rather than sitting dead-center on the
    // screen — a fixed tilt per star, alternating left/right, that stays
    // applied while shown instead of straightening out on hover.
    const stars = graphicGroup.querySelectorAll('.constellation-star');
    const panel = document.getElementById('graphic-preview-panel');
    const panelImg = document.getElementById('graphic-preview-img');
    if (!panel || !panelImg) return;

    const TILTS = [-5, 4, -3, 5, -4, 3, -6, 4, -3, 5, -4, 6];
    let lockedStar = null;

    function positionPanel(star) {
        const rect = star.getBoundingClientRect();
        const gap = 22;
        const panelW = panel.offsetWidth || 260;
        const panelH = panel.offsetHeight || 320;
        const idx = parseInt(star.getAttribute('data-idx'), 10);

        // Alternate which side each star pops out to (by index) instead of
        // always preferring the right — falls back to the other side only
        // if the preferred one would overflow the viewport.
        const preferRight = idx % 2 === 0;
        let left = preferRight ? rect.left + gap : rect.left - gap - panelW;
        const fits = left >= 12 && left + panelW <= window.innerWidth - 12;
        if (!fits) {
            left = preferRight ? rect.left - gap - panelW : rect.left + gap;
        }
        ...
        panel.style.setProperty('--tilt', `${TILTS[idx % TILTS.length]}deg`);
    }

    function showImage(star) {
        const file = star.getAttribute('data-img');
        panelImg.src = encodeURI(`images/Graphics Portfolio/${file}`);
        panelImg.alt = file.replace(/\.[a-zA-Z]+$/, '');
        positionPanel(star);
    ...
    stars.forEach(star => {
        star.addEventListener('mouseenter', () => showImage(star));

        star.addEventListener('mouseleave', () => {
            if (lockedStar) {
                showImage(lockedStar);
            } else {
                hidePanel();
            }
        });
        ...
        star.addEventListener('click', () => {
            if (lockedStar && lockedStar !== star) {
                lockedStar.classList.remove('is-locked');
            }
            star.classList.add('is-locked');
            lockedStar = star;
            showImage(star);
        });
    });
